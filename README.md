...
Протокол генерирует системные уведомления о ходе работы и ошибках.

События:

* `info` — информационные сообщения
* `warn` — предупреждения
* `error` — ошибки во время работы

Пример:

```js
protocol.on('info', (reason) => console.log('INFO:', reason.code));
protocol.on('warn', (reason) => console.warn('WARN:', reason.code));
protocol.on('error', (reason) => console.error('ERROR:', reason.code));
```

Структура объекта `reason`

```js
{
    name: "ReportWarn",
    code: "LANG_NOT_FOUND",
    message: "LANG_NOT_FOUND\nВыбранный язык 'en' не найден.\n...",
    stack: "stack trace...",
    details: [
        "Выбранный язык 'en' не найден.",
        "Изменено: 'en' → 'ru'",
...
Если во время создания протокола возникает критическая ошибка,
конструктор выбрасывает исключение.

Это не событие и не может быть перехвачено через `protocol.on(...)`.

Пример:

```js
try {
    const protocol = new Protocol({ channel: { name: 'unknown' } });
} catch (reason) {
    console.error('INIT_ERROR:', reason.code);
}
```
...
Протокол поддерживает локализацию системных уведомлений (ошибок, предупреждений и информационных сообщений).
...
* будет отправлено событие `warn`
* будет автоматически выбран язык по умолчанию
...
1. Создайте файл в `lib/langs/`, например `lib/langs/en.js`
2. Добавьте язык в `langs.supported(...)` в файле `lib/langs/langs.js`

### Как язык влияет на события

События `info`, `warn`, `error` формируются с учетом выбранного языка.
...
│
├── assembly/
│   ├── assembler.js          # Инициализатор сборки
│   └── rules/
│       ├── registry/         # Список правил сборки реестра контрактов
│       │   └── scope.js      # Правило инициализации области инициализации
│       │
│       ├── langs/            # Список правил сборки языков
│       │   ├── scope.js      # Правило инициализации области инициализации
│       │   └── lang.js       # Правило инициализации языка
│       │
│       ├── channels/         # Список правил сборки каналов связи
│       │   ├── scope.js      # Правило инициализации области инициализации
│       │   └── channel.js    # Правило инициализации канала связи
│       │
│       └── engines/          # Список правил сборки движков
...
│
├── registry/
│   ├── packets.js            # Контракт структуры пакетов протокола
│   ├── channels.js           # Контракт каналов связи (Protocol ↔ Channel)
...
│
├── langs/
│   ├── langs.js              # Список поддерживаемых языков
│   └── ru.js                 # Русский
│
├── channels/
│   ├── channels.js           # Список поддерживаемых каналов связи
│   └── l2raw/                # Канал связи на уровне L2
│       ├── config.js         # Конфигурация канала связи
...
└── engines/
    ...
    │   └── logic/
    │       ├── director.js   # Логика инициализации выбранного языка
    ...
    │   └── logic/
    │       ├── director.js   # Логика инициализации выбранного канала связи
    │       ├── activator.js  # Логика активации выбранного канала связи
    │       └── builder.js    # Логика сборки выбранного канала связи
    │
    └── io/
